...
## When cleanup will not complete

**The cleanup failure paths live in `../references/traps.md#cleanup` and load only when one fires:** `git branch -d` refusing (it has two predicates, and neither refusal means `-D`), `git worktree remove` failing for a reason that is not a git refusal, a removal command reporting success over work it did not do, or the default branch being unreachable so that cleanup can only be partial — which is a terminal state, not a failure.
